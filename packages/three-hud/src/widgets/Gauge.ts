...
export class Gauge extends HudNode {
  ...
  constructor(options: GaugeOptions = {}) {
    ...
    this.face = this.add(
      new Ring({
        ...
        innerRadius: this.size.width / 2 - 8,
    ...
    this.needle = this.add(
      new Line({ id: `${this.id}-needle`, width: 4, height: this.size.height / 2, fill: 0xff6688 }),
    );
  ...
  private syncNeedle(): void {
    ...
    const angle = this.face.startAngle + this.face.progressSweep();
    this.needle.rotation = angle;
    this.needle.setPosition(this.size.width / 2, this.size.height / 2);
    void ratio;
  }
}
